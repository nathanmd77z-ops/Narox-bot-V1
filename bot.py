...
class TicketReasonModal(discord.ui.Modal, title="Ouvrir un ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="Utilise les boutons ci-dessous pour gérer le ticket.")
...
class TicketManagementView(discord.ui.View):
    ...
    async def claim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_staff_for_ticket(interaction.user, ticket_type):
            return await interaction.response.send_message(
                "Tu n'as pas la permission de claim ce ticket.",
                ephemeral=True
            )
    ...
    async def unclaim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_staff_for_ticket(interaction.user, ticket_type):
            return await interaction.response.send_message(
                "Tu n'as pas la permission de unclaim ce ticket.",
                ephemeral=True
            )

        if meta["claimed_by"] is None:
            return await interaction.response.send_message(
                "Ce ticket n'est pas claim.",
                ephemeral=True
            )
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="Ticket fermé",
            description=f"{interaction.user.mention} a fermé le ticket `{channel.name}`",
